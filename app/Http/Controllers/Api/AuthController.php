...
class AuthController extends Controller
{
    ...
    public function __construct(transferRequest $transferRequest, Auth $auth, Validate $validate, User $user, sendMail $mail, ResetPassword $reset, Partials $partials, TranxConfirm $trnx, apiHelper $api, Stash $stash, Referral $referral, Lender $investor, Portfolio $portfolio, Investment $investment, Transaction $transaction, Funds $fund, Saving $saving, fundPayment $payment, loanRates $rates, Reserve $reserve, Safe $safe, Formatter $formatter)
    {
        $this->auth = $auth;
        $this->user = $user;
        $this->mail = $mail;
    ...
    public function login(Request $request)
    {
        ...
        if ($validation->fails()) {
            ...
            return response()->json(['status' => 'error', "message" => $validation->getMessageBag()], 200);
        ...
        try {
            ...
            if ($this->auth::user()->verified == false) {
                Auth::logout();
                return response()->json(['status' => 'error', "message" => "Your account is not verified. Kindly check your email for a verification link"], 200);
            ...
            if ($token) {
                //\Log::info(print_r($this->auth::user()));
                try {
                    $user = $this->auth::user();
                    $stash = $this->stash->where('investorId', $user->investor()->id);
                    //\Log::info(print_r($stash));
                    if ($stash->first() === null) {
                        $availableBalance = 0;
                        $totalAmount = 0;
                        $isStash = false;
